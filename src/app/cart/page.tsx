...
const CartPage = () => {
  ...
  const deleteCartItemFromList = async (id: string) => {
    const test = cart.filter((item) => item.id === id);

    await CartApis.deleteCartItem(id)
      .then((res) => {
        if (res)
          setCart((oldCart) =>
            oldCart.filter((item) => item.id !== test[0].id)
          );
      })
      .catch((error) => {
              ...
              {cart?.map((item) => (
                <li className="flex items-center gap-4" key={item?.id}>
                  ...
                  <div className="flex items-center justify-end flex-1 gap-2">
                    ${item?.product?.price}
                    <button
                      onClick={() => deleteCartItemFromList(item?.id as string)}
